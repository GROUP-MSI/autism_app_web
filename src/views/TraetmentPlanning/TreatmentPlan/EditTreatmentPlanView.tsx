...
import { updateTreatmentPlan, type TreatmentPlan } from "../../../store/slices/treatmentPlanSlice"
import { toast } from "react-toastify"

interface EditTreatmentPlanViewProps {
  plan: TreatmentPlan
  onClose: () => void
}

export const EditTreatmentPlanView = ({ plan, onClose }: EditTreatmentPlanViewProps) => {
  const dispatch = useDispatch()
  const [formData, setFormData] = useState({
    title: plan.title,
    description: plan.description,
    childName: plan.childName,
    childAge: plan.childAge.toString(),
    ...
    completedSessions: plan.completedSessions.toString(),
    status: plan.status,
  })
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const updatedPlan: TreatmentPlan = {
      ...
      completedSessions: Number.parseInt(formData.completedSessions),
      status: formData.status as TreatmentPlan["status"],
      updatedAt: new Date().toISOString(),
    }

    dispatch(updateTreatmentPlan(updatedPlan))
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-xl font-bold text-gray-900">Editar Plan de Tratamiento</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  setFormData((prev) => ({
                    ...prev,
                    status: e.target.value as "draft" | "active" | "completed" | "paused",
                  }))
                }
                className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
              >
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">Total Sesiones</label>
              <input
                type="number"
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => setFormData((prev) => ({ ...prev, completedSessions: e.target.value }))}
                className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                min="0"
